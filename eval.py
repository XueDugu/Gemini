...
if __name__ == '__main__':
    ...
    Gs_train, classes_train, Gs_dev, classes_dev, Gs_test, classes_test = partition_data(Gs, classes, [0.8, 0.1, 0.1], perm)
    ...
    # Model
    gnn = graphnn(N_x=NODE_FEATURE_DIM, Dtype=Dtype, N_embed=EMBED_DIM, depth_embed=EMBED_DEPTH,
                  N_o=OUTPUT_DIM, ITER_LEVEL=ITERATION_LEVEL, lr=LEARNING_RATE)
    gnn.init(LOAD_PATH, LOG_PATH)

    # Test
    val_auc, fpr, tpr, thres = get_auc_epoch(gnn, Gs_dev, classes_dev, BATCH_SIZE, load_data=valid_epoch)
    print("AUC on validation set: {}".format(val_auc))
    test_auc, fpr, tpr, thres = get_auc_epoch(gnn, Gs_test, classes_test, BATCH_SIZE, load_data=test_epoch)
    print("AUC on testing set: {}".format(test_auc))
